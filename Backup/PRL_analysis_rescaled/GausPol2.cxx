...
// ----------------------------------------------------------------------------------------------------- CONSTRUCTOR
GausPol2::GausPol2(const std::string& name, std::vector<double> bin_content, int E0, int xL, int xR, double E1, double E2, int outputK, int *rng)
    : BCModel(name)
{
	    ...
	    sprintf(namefile,"MarginalizedRESCALED/53_93_BEGe_PSD_ConstantP/%i.root", E0);
	    TFile *file = new TFile(namefile);
	    sprintf(hName_E0, "h_E0_new");
	    ...
	    if ( outputK<=1 || outputK==4 || outputK==7 || (outputK>=13 && outputK<=15) || outputK==18 || outputK>=20 ) {
		    ...
		    AddParameter("E0_height", 0, max_height[3]+xE0, "", "[(events*#epsilon) / (kg*yr)]");
