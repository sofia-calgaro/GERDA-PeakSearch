...
readonly a=0.681 # BEGe (pre upgrade)
readonly b=0.000427
#readonly a=0.551 # BEGe
#readonly b=0.0004294
#readonly a=1.025 # COAX (pre upgrade)
#readonly b=0.000647
...
thr=195
thr_pol=195
norm=0 # 0 = for NOT-normalized spectra
       # 1 = otherwise
...
do
	...
	# rough way to solve the problems of numbers with 0 after the comma
	xL=`printf "%.0f" $xL`
	xR=`printf "%.0f" $xR`
	...
	ckE0=1 # check for hE0 range
	ckE1=1 # check for hE1 range
	ckE2=1 # check for hE2 range
	...
	do
		warnings=3200000000000000
		while [ "$warnings" -gt 2200000000000000 ]
		do
			warnings=0
			./runDataAnalysis --nums 13 "$E0" "$pol_degree" "$xL" "$xR" "$k" "$outputK" "$rng0" "$rng1" "$rng2" "$rngE0" "$rngE1" "$rngE2" "$thr" &&
		...
		if [[ "$rng0" -gt 16 || "$rng1" -gt 16 || "$rng2" -gt 16 || "$rngE0" -gt 11 || "$rngE1" -gt 11 || "$rngE2" -gt 11 ]]
		then
			break
		fi	
