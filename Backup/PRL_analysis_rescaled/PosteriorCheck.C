...
void PosteriorCheck(int E0, int pol_degree, int outputK, const char *root_file) {
	...
	char name[200];
	sprintf(name,"MarginalizedROOT/53_93_coax_PSD_BEGeP_re/%i.root", E0);
	...
	double p0FirstBin=0, p0LastBin=0, p1FirstBin=0, p1LastBin=0, p2FirstBin=0, p2LastBin=0;
	...
	if ( pol_degree==2 ) {
		...
		p2BinMax = hp2->GetMaximumBin();
	}
	
	double E0LastBin=0, E1LastBin=0, E2LastBin=0;
	...
	for ( int i=1; i<=5; i++ ) {
		...
		if ( pol_degree==1 ) { p1FirstSum += hp1->GetBinContent(i); }
		if ( pol_degree==2 ) { 
			p1FirstSum += hp1->GetBinContent(i);
			p2FirstSum += hp2->GetBinContent(i);
		}
	}
	p0FirstMean = p0FirstSum/5;
	if ( pol_degree==1 ) { p1FirstMean = p1FirstSum/5; }
	if ( pol_degree==2 ) { 
		p1FirstMean = p1FirstSum/5; 
		p2FirstMean = p2FirstSum/5; 
	}
	
	// Mean of last 5 bins 
	double p0LastSum=0, p1LastSum=0, p2LastSum=0;
	double p0LastMean=0, p1LastMean=0, p2LastMean=0;
	for ( int i=(hp0->GetNbinsX())-4; i<=hp0->GetNbinsX(); i++ ) { p0LastSum += hp0->GetBinContent(i); }
	if ( pol_degree==1 ) { for ( int i=(hp1->GetNbinsX())-4; i<=hp1->GetNbinsX(); i++ ) { p1LastSum += hp1->GetBinContent(i); } }
	if ( pol_degree==2 ) { 
		for ( int i=(hp1->GetNbinsX())-4; i<=hp1->GetNbinsX(); i++ ) { p1LastSum += hp1->GetBinContent(i); } 
		for ( int i=(hp2->GetNbinsX())-4; i<=hp2->GetNbinsX(); i++ ) { p2LastSum += hp2->GetBinContent(i); } 
	}
	p0LastMean = p0LastSum/5;
	if ( pol_degree==1 ) { p1LastMean = p1LastSum/5; }
	if ( pol_degree==2 ) { 
		p1LastMean = p1LastSum/5;
		p2LastMean = p2LastSum/5; 
	}
