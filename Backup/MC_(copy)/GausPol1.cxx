...
// ----------------------------------------------------------------------------------------------------- CONSTRUCTOR
GausPol1::GausPol1(const std::string& name, std::vector<double> bin_content, int E0, int xL, int xR, double E1, double E2, int outputK, int *rng, int det, int phase, int norm, int PSD, int prior)
    : BCModel(name)
{
	    ...
	    if ( outputK<=1 || outputK==4 || outputK==7 || outputK==13 || outputK==14 || outputK==15 || outputK==18 || outputK>=20 ) {
		    double *output_pol1 = FindRange_Pol1( E0, bin_content, max_height, det, phase);
		    ...
		    double *output_G_pol1 = FindRange_Gamma_Pol1( E0, bin_content, max_height, E1, max_gammaYield, xL, xR, det, phase);
		    ...
		    double *output_2G_pol1 = FindRange_TwoGamma_Pol1( E0, bin_content, max_height, E1, max_gammaYield1, E2, max_gammaYield2, xL, xR, det, phase);
...
double GausPol1::LogLikelihood(const std::vector<double>& pars)
{
	    ...
	    int counts = 0;
            ...
            for ( int i=xL; i<xR; i++ ) {
                    ...
                    // expected value
                    if ( outputK<=1 || outputK==4 || outputK==7 || outputK==13 || outputK==14 || outputK==15 || outputK==18 || outputK>=20 ) {
                    	y_exp =  pars[0]*TMath::Gaus(i, E0, FindSigma(E0, det, phase), true) + pars[1] + pars[2]*(i-E0);
                    }
                    else if ( outputK==2 || outputK==3 || outputK==5 || outputK==6 || outputK==12 ) { 
                    	double E1 = GetDataSet()->GetDataPoint(5204).GetValue(0);
                    	y_exp =  pars[0]*TMath::Gaus(i, E0, FindSigma(E0, det, phase), true) + pars[1] + pars[2]*(i-E0) + pars[3]*TMath::Gaus(i, E1, FindSigma(E1, det, phase), true);
                    }
                    else { 
                    	double E1 = GetDataSet()->GetDataPoint(5204).GetValue(0);
                    	double E2 = GetDataSet()->GetDataPoint(5206).GetValue(0);
                    	y_exp =  pars[0]*TMath::Gaus(i, E0, FindSigma(E0, det, phase), true) + pars[1] + pars[2]*(i-E0) + pars[3]*TMath::Gaus(i, E1, FindSigma(E1, det, phase), true) + pars[4]*TMath::Gaus(i, E2, FindSigma(E2, det, phase), true);
                    }
