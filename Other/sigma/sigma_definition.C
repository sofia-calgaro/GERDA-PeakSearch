...
int sigma_definition() {
	...
	char filename2[200];
	if ( ans0==0 ) { sprintf(filename2, "/home/sofia/Analysis/Data2/ReadJSON/coax/GlobalMode.txt"); }
	...
	for ( int E0=E0_min; E0<=E0_max; E0++) {
		...
		char filename[200];
		if ( ans0==0 ) { sprintf(filename, "/home/sofia/Analysis/Data2/MarginalizedROOT/coax/%i.root", E0); }
		...
		// no gamma peaks
		if (E0<=119) { sprintf(filename3, "h1_%iGausPol2_parameter_E0_height", E0); }
		if (E0>=120 && E0<=229) { sprintf(filename3, "h1_%iGausPol1_parameter_E0_height", E0); }
		...
		if ( GlobalMode/qt_34 > 1 ) {
			...
			char filename4[200];
			if ( ans0==0 ) { sprintf(filename4, "/home/sofia/Analysis/Data2/ReadJSON/coax/68Quantile.txt"); }
	...
	if ( ans0==0 ) { legend->AddEntry(h_sigma, "EnrCoax (II, II+)", "l"); }
	if ( ans0==1 ) { legend->AddEntry(h_sigma, "EnrBEGe (II, II+)", "l"); }
	if ( ans0==2 ) { legend->AddEntry(h_sigma, "IC (II+)", "l"); }
