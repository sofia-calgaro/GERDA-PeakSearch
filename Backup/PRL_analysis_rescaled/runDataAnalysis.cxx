...
{  
    ...
    // output folder
    std::string foldername = "53_93_coax_PSD_BEGeP_re";
      
    //---------------------------------------------------------------------------------------------------------------------- DATA LOADING   
    //TFile *file = new TFile("/home/sofia/gerda_data/53_93_PSD/normalized_phII_PSD.root","READ"); // normalized spectrum + only PSD
    TFile *file = new TFile("/home/sofia/gerda_data/53_93_PSD/rescaled_phII_PSD.root","READ"); // normalized spectrum
    //TFile *file = new TFile("/home/sofia/gerda_data/53_93_PSD/IC/20210726_IC.root","READ");
    //TFile *file = new TFile("/home/sofia/Analysis/PRL_analysis/PRL_article_csv/Rizalina_NN_spectra.root","READ");
    
    TH1D *h = (TH1D*) file->Get("hCOAX"); // normalized spectrum
    //TH1D *h = (TH1D*) file->Get("LAr"); // normalized spectrum
    //TH1D *h = (TH1D*) file->Get("LAr_PSD");
    //TH1D *h = (TH1D*) file->Get("h_bege_NN_Riz");
    std::vector<double> bin_content;
    for ( int i=1; i<=5200; i++ ) { bin_content.push_back( h->GetBinContent(i) ); }
        		
    // create a new dataset
    BCDataSet data_set;
    data_set.ReadDataFromFileTxt("/home/sofia/gerda_data/53_93_PSD/rescaled_phII_PSD_COAX.txt", 1); // normalized spectrum + only PSD
    //data_set.ReadDataFromFileTxt("/home/sofia/gerda_data/53_93_PSD/normalized_phII_PSD_BEGe.txt", 1); // normalized spectrum 
    //data_set.ReadDataFromFileTxt("/home/sofia/Analysis/PRL_analysis/PRL_article_csv/Rizalina_bege_NN.txt", 1);
    ...
    std::fstream output_file_test;
    output_file_test.open("test_limits.txt", std::fstream::app);
    int k = inpval[5];
    ...
    output_file_test << E0 << "\t\t" << E1 << "\t\t" << outputK << std::endl;
    ...
    //char name_log[100];
    //sprintf(name_log,"Log_files/log_%i_GausPol%i.txt", E0, pol_degree);
    ...
    if ( pol_degree==0 ) {
	    ...
	    m.PrintAllMarginalized("MarginalizedPDF/" + foldername + "/" + m.GetSafeName() + "_plots.pdf", 2, 2);
	    m.PrintKnowledgeUpdatePlots("Priors&Posteriors/" + foldername + "/" + m.GetSafeName() + "_plots.pdf", 2, 2);
	    m.PrintSummary();
	    m.WriteMarginalizedDistributions("MarginalizedROOT/" + foldername + "/" + std::to_string(E0) + ".root", "RECREATE");
    ...
    if ( pol_degree==1 ) {
	    ...
	    BCFitter* models[] = { &m };
    	    models[0]->SetFitFunctionIndices(0, 1);
    	    models[0]->SetFillErrorBand();
    	    
	    TGraph* g_err = models[0]->GetErrorBandGraph(0.16, 0.84);
	    trash.Put(g_err);
            g_err->SetFillColor(1);
            g_err->SetFillStyle(3002);
            g_err->Draw("f");
            
            TGraph* g_best = models[0]->GetFitFunctionGraph();
            trash.Put(g_best);
            g_best->SetLineColor(2);
            g_best->SetFillColor(2);
            g_best->SetFillStyle(3002);
            g_best->Draw("c same");
            TLegend leg(0.2, 0.60, 0.6, 0.8);
    	    leg.SetFillColor(kWhite);
    	    leg.SetBorderSize(0);
            leg.AddEntry(g_best, "Best Fit", "LF");
            leg.Draw();
            c->SaveAs("prova.png");*/
