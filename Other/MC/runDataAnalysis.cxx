...
{  
    ...
    //---------------------------------------------------------------------------------------------------------------------- DATA LOADING   
    char MCname[200];
    if ( det==0 ) sprintf(MCname, "/home/sofia/Analysis/MC/MC_spectra/coax/MCspectrum_%i.root", inpval[1]);
    if ( det==1 ) sprintf(MCname, "/home/sofia/Analysis/MC/MC_spectra/BEGe/MCspectrum_%i.root", inpval[1]);
    if ( det==2 ) sprintf(MCname, "/home/sofia/Analysis/MC/MC_spectra/IC/MCspectrum_%i.root", inpval[1]);
    TFile *file = new TFile(MCname,"READ");
    TH1D *h = (TH1D*) file->Get("LAr_MC");
    std::vector< int> bin_content;
    for ( int i=1; i<=5200; i++ ) { bin_content.push_back( h->GetBinContent(i) ); }
        		
    // create a new dataset
    BCDataSet data_set;
    data_set.ReadDataFromFileTxt("bin_content_MC.txt", 1);
    ...
    char name_log[100];
    sprintf(name_log,"/home/sofia/Analysis/MC/Log_files/log_%i_GausPol%i.txt", E0, pol_degree);
    ...
    if ( pol_degree==0 ) {
	    ...
	    m.PrintAllMarginalized("/home/sofia/Analysis/MC/MarginalizedPDF/" + m.GetSafeName() + "_plots.pdf", 2, 2);
	    m.PrintSummary();
	    char rootname[200];
	    sprintf(rootname, "/home/sofia/Analysis/MC/MarginalizedROOT/%i.root", E0);
	    m.WriteMarginalizedDistributions(rootname, "RECREATE");
