    ...
    : BCModel(name)
{
	    ...
	    sprintf(namefile,"MarginalizedROOT/53_93:_NORM_BEGe_PSD_ConstantP_Riz/%i.root", E0);
...
double GausPol0::LogLikelihood(const std::vector<double>& pars)
{
		    ...
		    // expected value
                    if ( outputK<=1 || outputK==4 || outputK==7 || outputK==13 || outputK==14 || outputK==15 || outputK==18 || outputK>=20 ) {
                    	y_exp =  pars[0]*TMath::Gaus(i, E0, FindSigma(E0), false) + pars[1];
                    }
                    else if ( outputK==2 || outputK==3 || outputK==5 || outputK==6 || outputK==12 ) {
                    	double E1 = GetDataSet()->GetDataPoint(5204).GetValue(0);
                    	y_exp =  pars[0]*TMath::Gaus(i, E0, FindSigma(E0), false) + pars[1] + pars[2]*TMath::Gaus(i, E1, FindSigma(E1), false);
                    }
                    else { 
                        double E1 = GetDataSet()->GetDataPoint(5204).GetValue(0);
                    	double E2 = GetDataSet()->GetDataPoint(5206).GetValue(0);
                    	y_exp =  pars[0]*TMath::Gaus(i, E0, FindSigma(E0), false) + pars[1] + pars[2]*TMath::Gaus(i, E1, FindSigma(E1), false) + pars[3]*TMath::Gaus(i, E2, FindSigma(E2), false);
                    }
