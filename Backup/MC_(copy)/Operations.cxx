...
double FindSigma(int energy, int det, int phase) {

   double a=0, b=0;
   
   if ( det==0 ) { // coax
   	if ( phase==0 ) { // pre-upgrade
	   	a = 1.025; 
	   	b = 6.47e-4; 
   	}
   	if ( phase==1 ) { // post-upgrade
	   	a = 0.898;
	   	b = 20e-4; 
   	}
   	//a = 0.985; // all phII
...
double FindFWHM(int energy, int det, int phase) {

   double FWHM = sqrt( 8*log(2) ) * FindSigma(energy, det, phase);
...
int *FindMaximumSignalHeight(int E0, double E1, double E2, std::vector<double> bin_content, int xL, int xR, int outputK, int det, int phase) {

   double FWHM_0 = FindFWHM(E0, det, phase);
   double FWHM_1 = FindFWHM(E1, det, phase);
   double FWHM_2 = FindFWHM(E2, det, phase);
...
int *FindMaximumGammaHeight1(int E0, double E1, double E2, std::vector<double> bin_content, int xL, int xR, int outputK, int det, int phase) {
...
int *FindMaximumGammaHeight2(int E0, double E1, double E2, std::vector<double> bin_content, int xL, int xR, int outputK, int det, int phase) {
...
double *FindRange_Pol0(int E0, std::vector<double> bin_content, int *max_height, int xL, int xR, int det, int phase) {
   ...
   for (int i=0; i<numGamma; i++) {
   	if ( E_gamma[i]>=x_min && E_gamma[i]<=xL ) {
   		...
   		if ( E_gamma[i+1]>=x_min && E_gamma[i+1]<=xL ) {
	   		if ( (E_gamma[i] - 1.5*FindFWHM(E_gamma[i], det, phase)) > x_min ) {
	   			for (int j=x_min; j<(E_gamma[i]-1.5*FindFWHM(E_gamma[i], det, phase)); j++) { B_i += bin_content.at(j); }
	   			N_bkg += ( (E_gamma[i]-1.5*FindFWHM(E_gamma[i], det, phase)) - x_min );
	   		}
	   		
	   		if ( (E_gamma[i+1] - E_gamma[i]) > 1.5*(FindFWHM(E_gamma[i], det, phase) + FindFWHM(E_gamma[i+1], det, phase)) ) { // far peaks
	   			for (int j=(E_gamma[i]+1.5*FindFWHM(E_gamma[i], det, phase)); j<(E_gamma[i+1]-1.5*FindFWHM(E_gamma[i+1], det, phase)); j++) { B_i += bin_content.at(j); }
	   			N_bkg += ( (E_gamma[i+1]-1.5*FindFWHM(E_gamma[i+1], det, phase)) - (E_gamma[i]+1.5*FindFWHM(E_gamma[i], det, phase)) );
	   		}
	   		
	   		if ( (E_gamma[i+1] + 1.5*FindFWHM(E_gamma[i+1], det, phase)) < xL ) {
		   		for (int j=(E_gamma[i+1]+1.5*FindFWHM(E_gamma[i+1], det, phase)); j<xL; j++) { B_i += bin_content.at(j); }
		   		N_bkg += ( xL - (E_gamma[i+1]+1.5*FindFWHM(E_gamma[i+1], det, phase)) );
	   		}
	   		break;
   		}
   		// 1 gamma peaks in [x_min;xL]
   			...
   			if ( (E_gamma[i] + 1.5*FindFWHM(E_gamma[i], det, phase)) < xL ) {
   				for (int j=(E_gamma[i]+1.5*FindFWHM(E_gamma[i], det, phase)); j<xL; j++) { B_i += bin_content.at(j); }
		   		N_bkg += ( xL - (E_gamma[i]+1.5*FindFWHM(E_gamma[i], det, phase)) );
   			}
   			break;
   		} 		
   	}
   ...
   for (int i=0; i<numGamma; i++) {	
   	if ( E_gamma[i]>=xR && E_gamma[i]<=x_max ) {
   		...
   		if ( E_gamma[i+1]>=xR && E_gamma[i+1]<=x_max ) {
	   		if ( (E_gamma[i] - 1.5*FindFWHM(E_gamma[i], det, phase)) > xR ) {
	   			for (int j=xR; j<(E_gamma[i]-1.5*FindFWHM(E_gamma[i], det, phase)); j++) { B_i += bin_content.at(j); }
	   			N_bkg += ( (E_gamma[i]-1.5*FindFWHM(E_gamma[i], det, phase)) - xR );
	   		}
	   		...
	   		if ( (E_gamma[i+1] + 1.5*FindFWHM(E_gamma[i+1], det, phase)) < x_max ) {
		   		for (int j=(E_gamma[i+1]+1.5*FindFWHM(E_gamma[i+1], det, phase)); j<x_max; j++) { B_i += bin_content.at(j); }
		   		N_bkg += ( x_max - (E_gamma[i+1]+1.5*FindFWHM(E_gamma[i+1], det, phase)) );
	   		}
	   		break;   		
   		}
   		// 1 gamma peaks in [xR;x_max]
   			...
   			if ( (E_gamma[i] + 1.5*FindFWHM(E_gamma[i], det, phase)) < x_max ) {
   				for (int j=(E_gamma[i]+1.5*FindFWHM(E_gamma[i], det, phase)); j<x_max; j++) { B_i += bin_content.at(j); }
		   		N_bkg += ( x_max - (E_gamma[i]+1.5*FindFWHM(E_gamma[i], det, phase)) );
   			}
   			break;
   		} 
   ...
   // ROOT fit: f0 = pol0 + gaus
	   ...
	   f0->FixParameter(3, FindSigma(E0, det, phase));   
...
double *FindRange_Pol1(int E0, std::vector<double> bin_content, int *max_height, int det, int phase) {
   ...
   for (int i=0; i<numGamma; i++) {
   	...
   	if ( x_min - E_gamma[i] < 2*FindFWHM(E_gamma[i], det, phase) && x_min - E_gamma[i] > 0 ) { x_min = std::round(x_min + FindFWHM(E_gamma[i], det, phase)); break; }	
   	// gamma peak == x_min
   	else if ( x_min == E_gamma[i] ) { x_min = std::round(x_min + 1.5*FindFWHM(E_gamma[i], det, phase)); }	
   	// gamma peak on the right
   	else if ( E_gamma[i] - x_min < 2*FindFWHM(E_gamma[i], det, phase) && E_gamma[i] - x_min > 0 ) { x_min = std::round(x_min - FindFWHM(E_gamma[i], det, phase)); break; }
   	// x_max:
   	// gamma peak on the left
   	if ( x_max - E_gamma[i] < 2*FindFWHM(E_gamma[i], det, phase) && x_max - E_gamma[i] > 0 ) { x_max = std::round(x_max + FindFWHM(E_gamma[i], det, phase)); break; }	
   	// gamma peak == x_min
   	else if ( x_max == E_gamma[i] ) { x_max = std::round(x_max + 1.5*FindFWHM(E_gamma[i], det, phase)); break; }	
   	// gamma peak on the right
   	else if ( E_gamma[i] - x_max < 2*FindFWHM(E_gamma[i], det, phase) && E_gamma[i] - x_max > 0 ) { x_max = std::round(x_max - FindFWHM(E_gamma[i], det, phase)); break; }
   }
   ...
   // ROOT fit: f1 = pol1 + gaus
	   ...
	   f1->FixParameter(4, FindSigma(E0, det, phase));   
...
double *FindRange_Pol2(int E0, std::vector<double> bin_content, int *max_height, int det, int phase) {
   ...
   // ROOT fit: f2 = pol2 + gaus
	   ...
	   f2->FixParameter(5, FindSigma(E0, det, phase));   
...
double *FindRange_Gamma_Pol0(int E0, std::vector<double> bin_content, int *max_height, double E1, int *max_gammaYield, int xL, int xR, int det, int phase) {
   ...
   if ( max_height[2] < 3*sqrt(max_height[1]) ) { 
	   ...
	   f0->FixParameter(3, FindSigma(E1, det, phase));
   ...
   // ROOT fit: f0 = pol0 + gaus(E1) + gaus
	   ...
	   f0->FixParameter(6, FindSigma(E0, det, phase));	   
...
double *FindRange_Gamma_Pol1(int E0, std::vector<double> bin_content, int *max_height, double E1, int *max_gammaYield, int xL, int xR, int det, int phase) {
   ...
   if ( max_height[2] < 3*sqrt(max_height[1]) ) { 
	   ...
	   f1->FixParameter(4, FindSigma(E1, det, phase));
   ...
   // ROOT fit: f1 = pol1 + gaus(E1) + gaus
	   ...
	   f1->FixParameter(7, FindSigma(E0, det, phase));	   
...
double *FindRange_Gamma_Pol2(int E0, std::vector<double> bin_content, int *max_height, double E1, int *max_gammaYield, int xL, int xR, int det, int phase) {
   ...
   if ( max_height[2] < 3*sqrt(max_height[1]) ) { 
	   ...
	   f2->FixParameter(5, FindSigma(E1, det, phase));
   ...
   // ROOT fit: f2 = pol2 + gaus(E1) +  gaus
	   ...
	   f2->FixParameter(8, FindSigma(E0, det, phase));   
...
double *FindRange_TwoGamma_Pol0(int E0, std::vector<double> bin_content, int *max_height, double E1, int *max_gammaYield1, double E2, int *max_gammaYield2, int xL, int xR, int det, int phase) {
   ...
   if ( max_height[2] < 3*sqrt(max_height[1]) ) { 
	   ...
	   f0->FixParameter(6, FindSigma(E2, det, phase));
   ...
   // ROOT fit: f0 = pol0 + gaus(E1) + gaus(E2) + gaus
	   ...
	   f0->FixParameter(9, FindSigma(E0, det, phase));	   
...
double *FindRange_TwoGamma_Pol1(int E0, std::vector<double> bin_content, int *max_height, double E1, int *max_gammaYield1, double E2, int *max_gammaYield2, int xL, int xR, int det, int phase) {
   ...
   if ( max_height[2] < 3*sqrt(max_height[1]) ) { 
	   ...
	   f1->FixParameter(7, FindSigma(E2, det, phase));
   ...
   // ROOT fit: f1 = pol1 + gaus(E1) + gaus(E2) + gaus
	   ...
	   f1->FixParameter(10, FindSigma(E0, det, phase));	   
...
double *FindRange_TwoGamma_Pol2(int E0, std::vector<double> bin_content, int *max_height, double E1, int *max_gammaYield1, double E2, int *max_gammaYield2, int xL, int xR, int det, int phase) {
   ...
   if ( max_height[2] < 3*sqrt(max_height[1]) ) { 
	   ...
	   f2->FixParameter(8, FindSigma(E2, det, phase));
   ...
   // ROOT fit: f2 = pol2 + gaus(E1) + gaus(E1) +  gaus
	   ...
	   f2->FixParameter(11, FindSigma(E0, det, phase));   
